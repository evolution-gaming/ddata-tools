 ...
 *
 * Akka Distributed Data is useful when you need to share data between nodes in an Akka Cluster. The
 * data is accessed with an actor providing a key-value store like API. The keys are unique
 * identifiers with type information of the data values. The values are Conflict Free Replicated
 * Data Types (CRDTs). All data entries are spread to all nodes, or nodes with a certain role, in
 * the cluster via direct replication and gossip based dissemination. You have fine grained control
...
trait SafeReplicator[F[_], A <: ReplicatedData] {
   ...
   *
   * @param consistency
   *   [[https://doc.akka.io/docs/akka/2.5.22/distributed-data.html#consistency]]
   */
  def get(
    implicit
